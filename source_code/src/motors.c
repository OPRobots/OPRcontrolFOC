...
static void init_absolute_position(void) {
  ...
  // motor_left_offset = map(get_encoder_left_absolute_position(), 0, 585, 0, 360);
  // motor_right_offset = map(get_encoder_right_absolute_position(), 0, 585, 0, 360);
...
void motors_move(void) {
  if (inited) {
    if (motor_left_speed != 0) {
      ...
      TIM_CCR3(TIM2) = sine_lookup[motor_left_A] * (abs(motor_left_speed) / 100.0f);
    ...
    if (motor_right_speed != 0) {
      ...
      TIM_CCR3(TIM1) = sine_lookup[motor_right_C] * (abs(motor_right_speed) / 100.0f);
